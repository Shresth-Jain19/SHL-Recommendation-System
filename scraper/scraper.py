...
def scrape_shl_catalog():
    ...
    for tab_num, CATALOG_URL in enumerate(CATALOG_URLS, 1):
            ...
            for i, row in enumerate(rows, 1):
                ...
                if adaptive_cell:
                    ...
                    if green_dot or ("green" in str(adaptive_cell).lower()):
                        adaptive_support = "Yes"  # Green circle emoji
                    else:
                        adaptive_support = "No"  # Red circle emoji
                ...
                if adaptive_text and adaptive_support == "NA":
                ...
                try:
                    ...
                    for heading in assessment_soup.find_all(['h2', 'h3', 'h4']):
                        ...
                        if remote_testing_text:
                            ...
                            if green_dot:
                                assessment_data["remote_testing"] = "Yes"  # Green circle emoji
                            else:
                                assessment_data["remote_testing"] = "No"  # Red circle emoji
                        else:
                            assessment_data["remote_testing"] = "NA"  # Question mark if section not found
        ...
        # save to csv
        with open("shl.csv", "w") as f:
            f.write("name,url,description,duration,languages,job_level,remote_testing,test_type,adaptive/irt_support,source_tab\n")
            for assessment in assessments:
                f.write(f"{assessment['name']},{assessment['url']},{assessment['description']},{assessment['duration']},{','.join(assessment['languages'])},{assessment['job_level']},{assessment['remote_testing']},{assessment['test_type']},{assessment['adaptive/irt_support']},{assessment['source_tab']}\n")
